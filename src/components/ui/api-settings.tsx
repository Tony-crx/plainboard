...
import { localStorageHelpers } from '@/lib/storage/indexeddb';
...
interface ApiSettingsModalProps {
   isOpen: boolean;
   onClose: () => void;
   onSave: (keys: string[], provider: ProviderType) => void;
   initialProvider?: ProviderType;
}

export function ApiSettingsModal({ isOpen, onClose, onSave, initialProvider = 'openrouter' }: ApiSettingsModalProps) {
   ...
   useEffect(() => {
      try {
         const stored = localStorageHelpers.getApiKeys(provider);
         if (stored.length > 0) setKeys(stored);
         else setKeys([]);
         setNewKey('');
      } catch (e) { }
   }, [isOpen, provider]);
   ...
   const addKey = () => {
      ...
      setKeys(updated);
      setNewKey('');
      localStorageHelpers.setApiKeys(provider, updated);
      onSave(updated, provider);
   };
            ...
            <div className="p-6 space-y-6">
               ...
               <div className="flex gap-2">
                  <button
                     ...
                     className={`flex-1 p-3 border font-bold uppercase tracking-widest text-xs transition-all ${provider === 'openrouter'
                  ...
                  <button
                     ...
                     className={`flex-1 p-3 border font-bold uppercase tracking-widest text-xs transition-all ${provider === 'groq'
                        ? 'border-red-500 bg-red-950/50 text-red-500'
                        : 'border-red-900/30 bg-[#0a0000] text-red-900 hover:border-red-900/60'
                        }`}
                  >
               ...
               <div className="flex gap-2">
                  <input
                     ...
                     placeholder={provider === 'openrouter' ? "sk-or-v1-..." : "gsk_..."}
               ...
               <div className="space-y-2 mt-4 font-mono text-sm max-h-[30vh] overflow-y-auto pr-2">
                  {keys.length === 0 && <div className="text-red-900/50 text-center py-4 uppercase tracking-widest text-xs">No local {provider} keys injected. System defaults active.</div>}
                  {keys.map((k, i) => (
